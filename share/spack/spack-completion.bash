...
_spack_develop() {
    if $list_options
    then
        SPACK_COMPREPLY="-h --help -p --path -b --build-directory --no-clone --clone --no-apply-changes --apply-changes -f --force -r --recursive"
    else
        _all_packages
    fi
}
...
_spack_undevelop() {
    if $list_options
    then
        SPACK_COMPREPLY="-h --help --no-apply-changes --apply-changes -a --all"
    else
        _all_packages
    fi
}
